...
export function InputBox({ variant, onSend, isStreaming, onStop, value: externalValue, onChange, responseStyle = 'normal', onStyleChange, memoryEnabled = false, onMemoryToggle, webSearchEnabled: extWebSearchEnabled, onWebSearchToggle }: InputBoxProps) {
  ...
  const model = providers
    .flatMap((p) => p.models)
    .find((m) => m.id === selectedModelId) || enabledModels[0]
  ...
  useEffect(() => {
    ...
    ta.style.height = Math.min(ta.scrollHeight, variant === 'home' ? 180 : 140) + 'px'
  }, [text, variant])
  ...
  return (
    <div className={`in-wrap${variant === 'home' ? '' : ''}`}>
      <div className={`in-box${variant === 'home' ? ' home-in-box' : ''}`}>
        ...
        <div className={variant === 'home' ? 'home-in-footer' : 'in-row-bottom'}>
          ...
          <div className="in-right">
            ...
            <ModelPicker isOpen={isModelPickerOpen} onClose={() => setIsModelPickerOpen(false)} triggerRef={modelChipRef} direction={variant === 'chat' ? 'up' : 'down'} />
            {isStreaming ? (
              <button className="send send-stopping" onClick={onStop} title="Stop generation" style={{ background: 'var(--danger-fill)', borderColor: 'var(--danger-fill)', color: 'var(--white)' }}>
